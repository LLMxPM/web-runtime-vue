...
-->
<template>
  <LatexViewer v-if="canRender" :content="resolvedContent" v-bind="$attrs" />
  <div v-else class="asset-formula__fallback" v-bind="$attrs">{{ fallbackMessage }}</div>
</template>
...
const fallbackMessage = computed(() => resolveAssetFallbackMessage(props.fallback, 'Formula 资源无法渲染，请检查资源名称或 LaTeX 内容。'))
