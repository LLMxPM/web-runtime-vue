      ...
      <nav class="flex-1 overflow-hidden">
        <div class="h-full overflow-y-auto overflow-x-hidden p-4 py-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent" :class="isCollapsed ? 'scrollbar-none' : ''">
          <ul class="list-none m-0 p-0">
            <li v-for="item in navigationItems" :key="item.path" :class="!isCollapsed ? 'mb-2' : 'mb-1'">
              <div v-if="item.children && item.children.length > 0" class="relative transition-all duration-300" :class="!isCollapsed ? 'bg-slate-100 rounded-2xl py-2 mx-3 border border-slate-200/80 shadow-sm' : ''">
                <div class="relative" @mouseenter="isCollapsed ? showSimpleTooltip(item.title, $event) : null" @mouseleave="isCollapsed ? hideSimpleTooltip() : null">
                  <div
                    ...
                    :class="[
                      ...
                      isCollapsed ? 'justify-center p-2 mx-2 text-base normal-case tracking-normal' : ''
                    ]" @click="handleNavClick(item)">
                    <div v-if="isCollapsed" class="flex items-center justify-center flex-shrink-0 font-bold text-[16px] text-gray-500 w-5 h-5">{{ item.title.charAt(0).toUpperCase() }}</div>
                    <span v-if="!isCollapsed" class="flex-1 whitespace-nowrap overflow-hidden text-ellipsis">{{ item.title }}</span>
                    <ChevronDown v-if="!isCollapsed && item.children.length > 0" class="ml-auto transition-transform duration-300 flex-shrink-0" :class="isMenuExpanded(item.path) ? 'rotate-180' : ''" :size="14" />
                  </div>
                </div>

                <transition name="submenu-slide">
                  <ul v-if="isMenuExpanded(item.path) && !isCollapsed" class="list-none mt-2 mb-0 mx-0 py-0 rounded-xl overflow-hidden bg-transparent p-0">
                    <li v-for="child in item.children" :key="child.path" class="m-0">
                      <PreviewThumbnail
                        :item="child"
                        :active="isActiveRoute(child.path)"
                        :page-config="previewPageConfig"
                        class="flex flex-col items-center text-[14px] p-1.5 mx-1.5 mb-2 bg-white border border-slate-200/80 shadow-sm hover:bg-gray-50"
                        @mouseenter="showSimpleTooltip(child.title, $event)"
              ...
              <template v-else>
                <PreviewThumbnail
                  ...
                  :page-config="previewPageConfig"
                  class="flex flex-col items-center mx-2 font-medium p-2 bg-white hover:bg-gray-50 border border-slate-200/80 shadow-sm"
                />
                <router-link
                  v-else
                  :to="item.path"
                  class="flex flex-col items-center text-gray-500 no-underline rounded-xl relative py-3 px-4 font-medium justify-center p-3 mx-2"
                  :class="isActiveRoute(item.path) ? 'bg-blue-500 text-white shadow-md shadow-blue-500/40' : 'border-transparent bg-transparent shadow-none'"
                  @mouseenter="showSimpleTooltip(item.title, $event)"
                  @mouseleave="hideSimpleTooltip()"
                >
                  <div class="flex items-center justify-center flex-shrink-0 font-bold text-[16px] w-5 h-5" :class="isActiveRoute(item.path) ? 'text-white' : 'text-gray-500'">{{ item.title.charAt(0).toUpperCase() }}</div>
                </router-link>
              </template>
            </li>
          </ul>
...
import { DEFAULT_PAGE_CONFIG, type RuntimeMenuMode } from '@/core/utils/config'
...
const previewPageConfig = computed(() => {
  const width = Number(props.appConfig.page?.width)
  const height = Number(props.appConfig.page?.height)
  return {
...
.submenu-slide-leave-from { max-height: 1500px; opacity: 1; transform: translateY(0); }
